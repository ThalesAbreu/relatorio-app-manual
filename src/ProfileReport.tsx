...
interface KpiData {
  custoPorVisualizacaoPerfil: number;
  cpc: number;
  cpm: number;
}
...
function ProfileReport() {
  ...
  const [visualizacoesPerfil, setVisualizacoesPerfil] = useState('');
  ...
  const [visualizacoesPerfilAnterior, setVisualizacoesPerfilAnterior] = useState('');
  ...
  const getPerformanceCommentary = (current: number, previous: number, metricName: string, lowerIsBetter: boolean = false): string => {
    ...
    const direction = current > previous ? 'aumento de' : 'redução de';
    const isImprovement = (lowerIsBetter ? current < previous : current > previous);

    if (isImprovement) {
        return `- ${metricName}: melhorou, com ${direction} ${Math.abs(diff).toFixed(2)}%.\n`;
    } else {
        return `- ${metricName}: teve uma variação, com ${direction} ${Math.abs(diff).toFixed(2)}%.\n`;
    }
  };
  ...
  const handleGenerateReport = (event: FormEvent) => {
    ...
    if (!periodo[0] || !periodo[1] || !investimento || !visualizacoesPerfil || !impressoes || !cliques) {
    ...
    const dataAtual: WeekData = {
      ...
      visualizacoesPerfil: parseInt(visualizacoesPerfil, 10),
      impressoes: parseInt(impressoes, 10),
      cliques: parseInt(cliques, 10),
    };
    
    const hasPreviousData = [investimentoAnterior, visualizacoesPerfilAnterior, impressoesAnterior, cliquesAnterior].some(f => f !== '');
    ...
    if (hasPreviousData) {
        ...
        const fieldsAnterior = [investimentoAnterior, visualizacoesPerfilAnterior, impressoesAnterior, cliquesAnterior];
        ...
        dataAnterior = {
            ...
            visualizacoesPerfil: parseInt(visualizacoesPerfilAnterior, 10),
    ...
    const calculateKpis = (data: WeekData): KpiData => ({
      custoPorVisualizacaoPerfil: data.visualizacoesPerfil > 0 ? data.investimento / data.visualizacoesPerfil : 0,
    ...
    const mainReport = [
        ...
        `Visitas ao Perfil: ${dataAtual.visualizacoesPerfil}`,
        `Custo por Visita ao Perfil: ${formatCurrency(kpiAtual.custoPorVisualizacaoPerfil)}`,
    ...
    if (dataAnterior) {
        ...
        let comments = getPerformanceCommentary(kpiAtual.custoPorVisualizacaoPerfil, kpiAnterior.custoPorVisualizacaoPerfil, 'Custo por Visita ao Perfil', true);
        ...
        comments += getPerformanceCommentary(dataAtual.visualizacoesPerfil, dataAnterior.visualizacoesPerfil, 'Visitas ao Perfil');
      ...
      <form onSubmit={handleGenerateReport}>
        <div className="form-columns">
          <fieldset>
            ...
            <input type="number" value={visualizacoesPerfil} onChange={(e: ChangeEvent<HTMLInputElement>) => setVisualizacoesPerfil(e.target.value)} placeholder="Visitas ao Perfil" className="form-input" />
          ...
          <fieldset>
            ...
            <input type="number" value={visualizacoesPerfilAnterior} onChange={(e: ChangeEvent<HTMLInputElement>) => setVisualizacoesPerfilAnterior(e.target.value)} placeholder={`Visitas ao Perfil Anterior`} className="form-input" />
